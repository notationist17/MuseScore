...
bool MuseScore::saveMLData(MasterScore * cs, const QString& saveName, const QString& partsName)
      {
      ...
      MasterScore* thisScore = cs->masterScore();
      ...
      for( Page* page: score->pages() ) {
            for( System* sys: page->systems() ) {
                  ...
                  for(const Element * e: elems) {
                        if (e->type() == Ms::ElementType::NOTE) {
                              Note * cur = (Note*)e;
                              int beg = cur->chord()->tick().ticks();
                              int end = beg + cur->chord()->actualTicks().ticks();
